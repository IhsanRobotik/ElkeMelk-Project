...
def main():
    ...
    while True:
        ...
        # Variables to store the bottommost 'bottle_open' center within the x-threshold
        ...
        if results and results[0].obb:
            for i, obb in enumerate(results[0].obb):
                ...
                if label == 'bottle_open':
                    ...
                    # Filter out detections with center_x less than the X_THRESHOLD
                    if center_x > X_THRESHOLD and center_y > 250:
                        ...
                        # Check if this object is the bottommost one within the x-threshold
                        if center_y > bottommost_y:
                            bottommost_y = center_y
                            bottommost_center = (center_x, center_y)

        # Output the bottommost 'bottle_open' object center above the X_THRESHOLD
        if bottommost_center:
            realX = bottommost_center[0] * conversion_factor
            realY = bottommost_center[1] * conversion_factor
            print("Bottommost 'bottle_open' Object Center (mm):", realX, realY)
