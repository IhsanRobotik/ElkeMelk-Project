...
# Load the YOLOv8 OBB model trained to detect 'bottle_open'
model = YOLO(r'path/to/model')  # Update with the path to your YOLOv8 OBB model file
model.to(device)
TARGET_CLASS_NAME = 'bottle_open'  # Define the target class name
...
def main():
    ...
    while True:
        ...
        if "trig" in msg:
            ...
            if array1[0] > 595:                                      #certain x border
                ...
                while True:    
                    ...
                    # Iterate over the results and find the leftmost detected object of the class 'bottle_open'
                    if results and results[0].boxes:
                        for i, box in enumerate(results[0].boxes):
                            # Filter by the target class 'bottle_open'
                            if box.cls == TARGET_CLASS_NAME:
                                try:
                                    x1, y1, x2, y2 = map(int, box.xyxy[0])  # Extracting coordinates
        ...
        cv.imshow("Detected Circle 1", annotated1_frame)
    ...
    # Reinitializing the second capture process
    ...
    if not cap.isOpened():
        return -1
    roi_x, roi_y, roi_w, roi_h = 0, 240, 1280, 210
